...
class AppAcessibilidade(ctk.CTk):
    ...
    def simular_calculo(self):
        ...
        try:
            ...
            self.visualizar_caminhos(cidade_selecionada, resultados, origem, destino)
    ...
    def visualizar_caminhos(self, cidade_nome, resultados, origem, destino):
        """ Cria a janela com o mapa e os 5 caminhos destacados """
        G = nx.Graph()
            
            # 1. Carregar todas as ligações do mapa (o fundo cinzento)
        for nodo, vizinhos in self.motor_mapa.adjacencias.items():
            for ligacao in vizinhos:
                G.add_edge(nodo, ligacao["destino"])
        ...
        nx.draw_networkx_nodes(G, pos, nodelist=[origem], node_color="green", node_size=500, label="Origem")
        nx.draw_networkx_nodes(G, pos, nodelist=[destino], node_color="red", node_size=500, label="Destino")
        ...
        curvaturas = [0.0, 0.5, -0.10, 0.8, -0.9]
        cores = ["#FFEE00", "#00FF62", "#007380", "#FFA500", "#E130A9"] # Vermelho, Azul, Verde, Laranja, Roxo
